...
install_omarchy() {
  ...
  if ! arch-chroot /mnt id "$OMARCHY_USER" >/dev/null 2>&1; then
    gum style "ERROR: User $OMARCHY_USER does not exist in the installed system"
    return 1
  fi
  ...
  if ! chroot_bash -lc "source /home/$OMARCHY_USER/.local/share/omarchy/install.sh"; then
    gum style "ERROR: Omarchy installer failed inside chroot"
    return 1
  fi
...
if [[ $(tty) == "/dev/tty1" ]]; then
  ...
  exec > >(tee >(sed -u 's/\x1b\[[0-9;]*[a-zA-Z]//g' >> "$OMARCHY_INSTALL_LOG_FILE") 2>/dev/null) 2>&1
  ...
  if [[ -f /tmp/omarchy_dualboot_mode ]]; then
    gum style "Dual Boot detected -- base system installed, proceeding to Omarchy installer"
    echo

    # Verify /mnt is mounted (configurator should have left it mounted)
    if ! findmnt -R /mnt >/dev/null 2>&1; then
      gum style "ERROR: /mnt not mounted. Dualboot installation may have failed."
      exit 1
    fi
  else
    install_arch
