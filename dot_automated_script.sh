...
run_configurator() {
  ...
  ./configurator
  export OMARCHY_USER="$(cat omarchy_username.txt 2>/dev/null || jq -r '.users[0].username' user_credentials.json)"
}
...
install_base_system() {
  ...
  sed -i \
    ...
    "/usr/lib/python${python_version}/site-packages/archinstall/lib/installer.py"
